...
//
// 使い方:
//   node scripts/sql.ts "SELECT * FROM days ORDER BY day_no"
//   node scripts/sql.ts "UPDATE items SET title='朝市散策' WHERE id=12"
//
...
if (!sql) {
  console.error('usage: node scripts/sql.ts "<SQL>"');
  process.exit(1);
}
