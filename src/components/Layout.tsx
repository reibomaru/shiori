...
const NAV = [
  { to: "/map", labelKey: "nav.map", Icon: FaMapLocationDot },
  { to: "/itinerary", labelKey: "nav.itinerary", Icon: FaRegCalendarDays },
  { to: "/budget", labelKey: "nav.budget", Icon: FaWallet },
...
export default function Layout() {
  ...
  const tripPeriod = (days: Day[], trip: TripMeta) => {
    ...
    return t("layout:period", { start, end });
  };
    ...
    <div className="mesh-light flex min-h-screen">
      ...
      >
        <div className="relative border-b border-white/10 px-5 py-5">
          ...
          <NavLink
            to="/"
            title={t("common:menu.toProjects")}
          ...
          <h1 className="mt-1 text-base font-bold leading-snug text-slate-100">{data?.trip?.title || project?.name || t("common:app.fallbackTitle")}</h1>
          {data?.trip && (
            <dl className="mt-3 space-y-1.5 text-xs text-slate-400">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{t("layout:party", { travelers: data.trip.travelers, count: data.trip.party_size })}</span>
              </div>
              <div className="flex items-center gap-2">
                <FaYenSign className="shrink-0 opacity-70" />
                <span>{t("layout:perPerson", { amount: yen(data.budget.reduce((s, b) => s + b.per_person, 0)) })}</span>
              </div>
            </dl>
          )}
        </div>
        ...
        <div className="space-y-2 border-t border-white/10 px-3 py-4">
          ...
          <div className="flex items-center gap-1 rounded-lg bg-white/5 px-1.5 py-1.5 ring-1 ring-inset ring-white/5">
            ...
            <Tooltip label={t("common:auth.logout")} side="top">
              <button
                onClick={() => void logout()}
      ...
      <main className={`mt-14 h-[calc(100dvh-3.5rem)] min-w-0 flex-1 overflow-y-auto md:mt-0 md:h-screen ${
        ...
        {error ? (
          <div className="mx-auto max-w-xl p-10 text-center">
            <p className="text-rose-600 dark:text-rose-400">{t("layout:apiError", { error })}</p>
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              <Trans
                i18nKey="layout:apiErrorHint"
                components={[<code className="rounded bg-slate-200 px-1 dark:bg-slate-700 dark:text-slate-200" />]}
              />
            </p>
          </div>
        ) : !data ? (
          <div className="p-10 text-center text-slate-400 dark:text-slate-500">{t("common:state.loading")}</div>
        ) : fullBleed ? (
          <Outlet />
        ) : (
