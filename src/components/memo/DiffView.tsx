...
export default function DiffView({ before, after }: { before: string; after: string }) {
  ...
  if (added === 0 && removed === 0) {
    return <p className="text-xs text-slate-400">本文の変更はありません。</p>;
  }

  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50 px-2.5 py-1 text-[10px] font-semibold">
      ...
      <div className="overflow-x-auto font-mono text-[11px] leading-relaxed">
        {segments.map((seg, idx) =>
          seg.kind === "gap" ? (
            <div
              key={idx}
              className="select-none border-y border-slate-100 bg-slate-50 px-2.5 py-0.5 text-[10px] text-slate-400"
            >
              ⋯ {seg.count} 行省略
            </div>
          ) : (
            <div key={idx} className={`flex ${ROW_STYLE[seg.row.type]}`}>
              <span className="w-8 shrink-0 select-none border-r border-slate-100 px-1 text-right text-slate-300">
                {seg.row.a ?? ""}
              </span>
              <span className="w-8 shrink-0 select-none border-r border-slate-100 px-1 text-right text-slate-300">
                {seg.row.b ?? ""}
              </span>
              <span className="w-4 shrink-0 select-none text-center text-slate-400">{SIGN[seg.row.type]}</span>
